...
from utils.cac_loss import CACLoss
...
parser.add_argument('--loss', default = 'supcon', type=str, choices = ['supcon', 'cider'],
                    help='train loss')
...
args.tb_path = './save/s2ca/{}_tensorboard'.format(args.in_dataset)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_sloss, train_uloss, train_dloss = train_cider(args, train_loader, model, criterion_supcon, criterion_comp, criterion_dis, optimizer, epoch, log,cac)
        if args.loss == 'supcon':
...
def train_cider(args, train_loader, model, criterion_supcon, criterion_comp, criterion_dis, optimizer, epoch, log,cac):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        penultimate_0 = model.encoder(input).squeeze()
        # s2ca need normalization
        penultimate = F.normalize(penultimate_0, dim=1)
        ...
        if args.loss == 'cider':
            ...
            tmp = penultimate_0[:bsz,:]
            logits = model.fc(tmp)
            dists = cac.distance_classifier(logits)
            losscac = cac.loss_fn(dists,target[:bsz])
            t = max(0, 1 - 0.02*epoch)
            loss = supcon_loss * t + (1-t) * losscac[0]
