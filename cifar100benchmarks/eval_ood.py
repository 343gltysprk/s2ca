...
def process_args():
    ...
    parser.add_argument('--loss', default = 's2ca', type=str, choices = ['supcon', 'cider','s2ca'],
...
def main(args):
    ...
    if args.score == 'maha':
        ...
        vim = ViM(net.intermediate_forward, d=128, w=net.fc.weight, b=net.fc.bias)
        vim.fit(train_loader, device='cuda')
        score_all = []
        tqdm_object = tqdm(test_loader, total=len(test_loader))
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(tqdm_object):
                scores = vim(images.to('cuda'))
                score_all.extend(scores)
        in_score = np.asarray(score_all, dtype=np.float32)
    ...
    for out_dataset in out_datasets:
        ...
        if args.score == "knn":
            ...
            score_all = []
            tqdm_object = tqdm(ood_loader, total=len(ood_loader))
            with torch.no_grad():
                for batch_idx, (images, labels) in enumerate(tqdm_object):
                    scores = vim(images.to('cuda'))
                    score_all.extend(scores)
            out_score = np.asarray(score_all, dtype=np.float32)
